...
function CIT() {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredTrainings = data.filter(training => training.main_training_name === "Cairn Introduction Training");
      setTrainings(filteredTrainings);
      const progressResponse = await fetch(`${BASE_URL}/api/employee/${employeeId}/main-training/3/`);
